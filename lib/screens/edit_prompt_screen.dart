...
class EditPromptScreen extends StatefulWidget {
  ...
  const EditPromptScreen({super.key, required this.prompt});
...
class _EditPromptScreenState extends State<EditPromptScreen> {
  ...
  late final TextEditingController _outputPreviewController;
  ...
  void initState() {
    ...
    _imageUrlController = TextEditingController(text: widget.prompt.imageUrl ?? '');
    _outputPreviewController = TextEditingController(text: widget.prompt.outputPreview ?? '');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: _isLoading ? null : _updatePrompt,
            child: _isLoading
                ? const SizedBox(
                    width: 20,
                    height: 20,
